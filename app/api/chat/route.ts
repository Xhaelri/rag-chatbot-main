...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      if (!documentCheck) {
        ...
        if (relevantDocuments.length > 0) {
          ...
          // Create formatted context with more details and preserve source ID
          docContext = relevantDocuments
            .map((doc, i) => {
              ...
              // Add sourceId as metadata for front-end extraction
    ...
    // --- REVISED ARABIC SYSTEM PROMPT (Enhanced for Card Rendering) ---
    const systemPrompt = `
    ...
    1. **إذا طلب المستخدم حرفي محدد بشكل صريح** (مثل: "أحتاج سباك"، "أريد كهربائي"، "دلني على نجار"):
        ...
        - **إذا وجدت قائمة:** 
          - اعرض الحرفيين بنص واضح يتضمن كل المعلومات المتاحة عنهم
          - تأكد من الاحتفاظ بتنسيق المستندات الأصلي مع بداية كل مستند بـ "--- المستند" ونهايته بـ "--- نهاية المستند" لكي يتمكن نظام العرض من استخراج البيانات وعرضها في بطاقات جذابة
          - لا تقم بإزالة أو تعديل العلامات المميزة للمستندات حتى يتمكن النظام من معالجتها
    ...
    4. **تنسيق الرد:**
        ...
        - عند تقديم قائمة حرفيين، حافظ على تنسيق المستندات الأصلي لكي يتمكن النظام من عرضها في بطاقات.
    
    5. **اللغة:**
        - تحدث باللغة العربية الفصحى الواضحة.
        
    6. **تذكر دائماً:**
        - لا تعدل بنية وتنسيق المستندات المسترجعة حتى يتمكن نظام العرض من عرضها في بطاقات جذابة.
        - يحتاج نظام العرض إلى علامات "--- المستند" و "--- نهاية المستند" للتعرف على البيانات.
